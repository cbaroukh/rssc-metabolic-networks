...
for plate_to_plot in plates_to_plot:
    ...
    if plate_to_plot in plate_types:
        ...
        print('Starting the AUC computation of ', plate_to_plot, ' plates.')
        ...
        for num_row, row in enumerate(row_names):
            ...
            for num_col, col in enumerate(col_names):
                ...
                for num_exp, exp_line in enumerate(exp_debut_lines_to_plot):
                    ...
                    area = trapz(df.iloc[exp_line + 11:exp_end_lines_to_plot[num_exp], num_row * 12 + num_col + 1]
                                 , df.iloc[exp_line + 11:exp_end_lines_to_plot[num_exp], 0])

                    AUCs_name.append(strain_name_rep)
                    AUCs[substrate].append(area)


        df_AUCs = pd.DataFrame.from_dict(AUCs, orient='index', columns=AUCs_name)
        #print(df_AUCs)

        df_AUCs.to_excel(outputfolder + outputfilename + '_AUCs' + plate_to_plot + '.xlsx', sheet_name=plate_to_plot)
